...
class BaseModel(ABC):
    ...
    def update_learning_rate(self, lr_policy, losses):
        """Update learning rates for all the networks; called at the end of every epoch"""
        for scheduler in self.schedulers:
            if lr_policy == 'plateau':
                scheduler.step(losses['G'])
            else:
                scheduler.step()
    ...
    def get_current_visuals(self):
        ...
        if self.opt.use_aux:
            visual_ret['fake_B'] = visual_ret['fake_B'][0]
    ...
    def load_networks(self, epoch):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                #         continue
                #     if key.startswith('model_initial'):
                #         key_split = key.split('.')
                #         key_split[1] = str(int(key_split[1]) - 1)
                #         state_dict_new['.'.join(key_split)] = state_dict[key]
                #         continue

...
def tensor2im(input_image, imtype=np.uint8):
    ...
    if not isinstance(input_image, np.ndarray):
        ...
        if image_numpy.shape[0] == 1:  # DW or VV or VH
...
def save_image(image_numpy, image_path):
    ...
    """
    image_pil = Image.fromarray(np.squeeze(image_numpy), mode='L')
...
def load_vgg16(f, device):
    input_channels  = 1    # number of bands the VGG16 was trained on
    ...
    '''
    state_dict = torch.load(f, map_location=device)['model_state_dict']
    new_layer = state_dict['encoder.0.weight'][:, :input_channels]
    state_dict['encoder.0.weight'] = new_layer
